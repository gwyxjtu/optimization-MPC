...
if __name__ == '__main__':
    ...
    # 读取输入excel
    try:
        load = pd.read_excel('input_720/1109/ht45/input_720h.xls')
    except BaseException as E:
        _logging.error('读取input_720h的excel失败,错误原因为{}'.format(E))
        raise Exception
    try:
        sto = pd.read_excel('input_720/1109/ht45/input_now_ht19.xls')
    except BaseException as E:
        _logging.error('读取input_now的excel失败,错误原因为{}'.format(E))
        raise Exception
    try:
        sto_end = pd.read_excel('input_720/1109/ht45/input_end.xls')
    ...
    # 写入输出Excel
    try:
        to_csv(dict_control,"1109/dict_opt_control_ht45_hydrogen70")
