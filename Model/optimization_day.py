...
LastEditTime: 2024-02-21 15:30:04
...
def OptimizationDay(parameter_json,load_json,begin_time,time_scale,storage_begin_json,storage_end_json):
    ...
    z_hp=[m.addVar(vtype=GRB.BINARY,name=f"z_e{t}")for t in range(period)]
    ...
    for i in range(period):
        ...
        m.addConstr(p_fc[i] + p_pur[i] + p_pv[i] == p_eb[i] + 300*z_hp[i] + p_load[i])
    ...
    for i in range(period):
        ...
        m.addConstr(t_b[i]==9.5-(1000/(2*np.pi*2.07*200*192))*gp.quicksum((g_gtw[j]-g_gtw_l[j])*g_func[i-j] for j in range(i+1)))
