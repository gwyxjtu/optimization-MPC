...
#取B矩阵和基变量目标函数系数cb的函数,由于之前用的是SAUBLow，这个函数暂时用不上，暂且保留
...
def extract_basis(model) -> ss.csr_matrix:
    ...
    row_indices, col_indices, values,cb= [], [], [],[]
    ...
    return ss.csr_matrix((values, (row_indices, col_indices)), shape=(m, m)),cb
...
def OptimizationDay(parameter_json,load_json,begin_time,time_scale,storage_begin_json,storage_end_json,model_type,z_file):
    ...
    Args:
        parameter_json (_type_): 输入config文件中读取的参数，设备效率等等
        load_json (_type_): 预测的负荷向量和光伏
        begin_time: 开始的时间
        time_scale (_type_): 计算的小时数，本次为24
        storage_begin_json (_type_): 初始端储能状态
        storage_end_json (_type_): 末端储能状态
        model_type：指定是LP还是MIP
        z_file：固定的零一量
    """
    modelType={
        'MIP':'MIP',
    ...
    c = 4200/3.6/1000 #水的比热容kwh/(吨*℃)
    period = time_scale

    # 初始化设备效率参数
    try:
        k_fc_p = parameter_json['device']['fc']['k_fc_p'][begin_time:period]#fc电效率
        k_fc_g = parameter_json['device']['fc']['k_fc_g'][begin_time:period]#fc热效率
        k_b=parameter_json['device']['battery']['k_b'][begin_time:period]#电池效率
        max_count_day=parameter_json['device']['ghp']['max_count_day']#调度周期里热泵开启的最大小时数
        k_eb = parameter_json['device']['eb']['k_eb'][begin_time:period]#锅炉效率
        ht_loss = parameter_json['device']['ht']['miu_loss']#水箱损失
        de_loss = parameter_json['device']['de']['miu_loss']#管道损失
    ...
    # 初始化容量参数
    try:
        m_ht = parameter_json['device']['ht']['water_max']#水箱里水的质量
        m_de = parameter_json['device']['de']['water_max']#管道里水的质量
        m_gtw = parameter_json['device']['gtw']['water_max']#地热井水的质量
        k_gtw_fluid=0.12#热泵参数
        p_fc_max = parameter_json['device']['fc']['power_max']#fc装机
        p_eb_max = parameter_json['device']['eb']['power_max']#锅炉装机
        p_hp_max = parameter_json['device']['ghp']['power_max']#热泵装机
    except BaseException as E:
    ...
    # 初始化负荷
    try:
        p_load = list(load_json['p_load'])[begin_time:period]
    ...
    if model_type==modelType['MIP']:
        ...
        z_b= [m.addVar(vtype=GRB.BINARY, name=f"z_b{t}") for t in range(begin_time,period)]# 电池充还是方，冲1方0,#if p_b>storageM, z_b=1,else z_b=0
    ...
    for i in range(period-begin_time):
        ...
        #约束写法会导致SAUBLow取的值不同，这样写g_fc有上下界，p_fc上下界卡死，反过来写则p_fc有上下界，g_fc上下界卡死。g_eb同理
        g_fc_p_cons[i]=m.addConstr(g_fc[i] <= p_fc_max*k_fc_g[i]/k_fc_p[i])
        eb_ub_con[i]=m.addConstr(g_eb[i] <= p_eb_max*k_eb[i])
        m.addConstr(soc_b[i] <= c_b_max)

        # 设备约束
        ## fc
        m.addConstr(p_fc[i] - k_fc_p[i] * h_fc[i] == 0)
        g_fc_h_cons[i]=m.addConstr(g_fc[i] - k_fc_g[i] * h_fc[i] == 0)
        ...
        ## eb
        m.addConstr(g_eb[i] - k_eb[i] * p_eb[i] == 0)
        #pv
        ...
        #battery
        m.addConstr(soc_b[i]-soc_b_l[i]==k_b[i]*z_p_b[i]+(-p_b[i]+storageM-z_p_b[i])/k_b[i])
    ...
    if model_type==modelType['MIP']:
        dict_control = {# 负荷
            ...
            'k_b':[k_b[i] for i in range(period-begin_time)],
            'k_eb':[k_eb[i] for i in range(period-begin_time)],
            'k_fc_g':[k_fc_g[i] for i in range(period-begin_time)],
            'k_fc_p':[k_fc_p[i] for i in range(period-begin_time)],
        }
        z_file={
        ...
        #这一段代码是之前计算B逆乘b的测试代码，写在这是因为需要优化的结果，还得是LP
        # c=m.getConstrs()
        # constr_names_to_indices = {
        ...
        judge_dict={
            ...
            'k_eb':[k_eb[i] if p_eb[i].x==0 else g_eb[i].x/p_eb[i].x for i in range(period-begin_time)],
            ...
            'k_fc_p':[k_fc_p[i] if h_fc[i].x==0 else p_fc[i].x/h_fc[i].x for i in range(period-begin_time)],
            ...
            'k_fc_g':[k_fc_g[i] if h_fc[i].x==0 else g_fc[i].x/h_fc[i].x for i in range(period-begin_time)],
            ...
            'soc_b_l':[soc_b_l[i].x for i in range(period-begin_time)],
            'soc_b_l_LOW':[soc_b_l[i].SAUBLow for i in range(period-begin_time)],
            'k_b_UP':[10000 if (soc_b_l[i].x-soc_b[i].x)==0 else min((p_b[i].SALBUP-storageM)/((soc_b_l[i].x-soc_b[i].x)),(soc_b_l[i].x-soc_b[i].x)/(p_b[i].SALBUP-storageM+0.001)) for i in range(period-begin_time)],
            'k_b':[k_b[i] if (soc_b_l[i].x-soc_b[i].x)==0 else min((p_b[i].x-storageM)/(soc_b_l[i].x-soc_b[i].x),(soc_b_l[i].x-soc_b[i].x)/(p_b[i].x-storageM)) for i in range(period-begin_time)],
            'k_b_LOW':[0 if (soc_b_l[i].x-soc_b[i].x)==0 else min((p_b[i].SAUBLow-storageM)/(soc_b_l[i].x-soc_b[i].x),(soc_b_l[i].x-soc_b[i].x)/(p_b[i].SAUBUP-storageM+0.001)) for i in range(period-begin_time)],
        }
